...
class TCNModel:
    def __init__(self, target_state: str, target_mandi: str, surrogate_state: str, surrogate_mandi: str, 
                 lag_days: int, training_start_date: str, recommendation_start_date: str, commodity: str) -> None:
        ''' Global constants '''
        self.RETRAIN_FREQUENCY: int = 15
        self.INPUT_SIZE: int = 60
        self.OUTPUT_SIZE: int = 30
        ...
        self.forecast_type: str = 'shortterm_models'
        ...
        self.model_dir: str = os.path.join(self.data_dir, 'model_data', self.forecast_type, 'tcn', self.mandi_unique_identifier)
        ...
        self.forecasted_data_dir: str =  os.path.join(self.data_dir, 'forecasted_data', self.forecast_type, 'tcn', self.mandi_unique_identifier)
    ...
    def clean_old_files(self) -> None:
        ...
        for model_id in range(self.PERTUBED_MODEL_COUNT):
            ...
            for root, dirs, files in os.walk(top=saved_scaler_path, topdown=False):
                ...
                for dir in dirs:
                    shutil.rmtree(os.path.join(root, dir))
            logging.debug(f'[model {model_id}] Removing persist data dir')
            ...
            for root, dirs, files in os.walk(top=model_processed_forecasts_dir, topdown=False):
                for file in files:
                    os.remove(path=os.path.join(root, file))
            logging.debug(f'[model {model_id}] Removing archive recommendatoins')
            archive_recommendation_dir: str = os.path.join(self.archive_recommendation_dir, f'model_{model_id}')
            for root, dirs, files in os.walk(top=archive_recommendation_dir, topdown=False):
                for file in files:
                    os.remove(path=os.path.join(root, file))
            logging.debug(f'[model {model_id}] Removing range of forecasts')
    ...
    def split_train_and_test_data(self, merged_prices_df: pd.DataFrame):
        ...
        train_data, test_data = np.array(merged_prices[:-(self.OUTPUT_SIZE), :]), np.array(merged_prices[-(self.OUTPUT_SIZE):, :]) 
    ...
    def merge_target_and_surrogates_prices(self, model_id: int) -> pd.DataFrame:
        ...
        target_df: pd.DataFrame = pd.read_csv(os.path.join(target_mandi_prices_loc))
        ...
        surrogate_df: pd.DataFrame = pd.read_csv(os.path.join(surrogate_mandi_prices_loc))
        ...
        rename_columns = ['DATE', self.target_mandi, self.surrogate_mandi]
        merged_prices_df = merged_prices_df.set_axis(rename_columns, axis = 'columns')
    ...
    def prepare_training_data(self, training_data):
        ...
        total: int = self.INPUT_SIZE + self.OUTPUT_SIZE
        training_input: list = list()
        training_output: list = list()
        for i in range(0, len(training_data) - total, 1):
            train_input_i = training_data[i: i + self.INPUT_SIZE]
            train_output_i = training_data[i + self.INPUT_SIZE: i + total, 0]
    ...
    '''
    #TODO: Call it when it is the training day, also the appending prices function is pending...
    def generate_pertubed_model_dataset(self) -> None:
        logging.info(msg='Generating pertubed model dataset')
        pertubed_files: list = list()
        ...
        for mandi_state, mandi_name in mandis:
            ...
            mandi_df['DATE'] = pd.to_datetime(mandi_df['DATE'], self.DATE_FORMAT)
            ...
            for model_id in range(1, self.PERTUBED_MODEL_COUNT):
                ...
                pertubed_model_df: pd.DataFrame = pd.read_csv(os.path.join(self.prices_dir, f'{mandi_state}_{mandi_name}_prices.csv'))
                pertubed_model_df['DATE'] = pd.to_datetime(pertubed_model_df['DATE'], self.DATE_FORMAT)
                current_idx: int = 0
                ...
                pertubed_model_input_file: str = os.path.join(self.prices_dir, f'{mandi_state}_{mandi_name}_prices_{model_id}.csv')
                ...
                # impute_file(src_file_path=pertubed_model_input_file, output_file_path=pertubed_model_interpolated_output_file) #TODO: uncomment while testing...

        interpolateFile(pertubed_files) # interpolate the pertubed files  #TODO: Based on what is decided... update later
    ...
    def setup_forecast_files_for_models(self):
        ...
        for model_id in range(0, self.PERTUBED_MODEL_COUNT):
            ...
            model_dir = os.path.join(self.raw_forecasted_dir, f'model_{model_id}')
            # We remove the day 1 as it is the oldest day in the past (loosing the 30 day moving window)
            if os.path.exists(path=os.path.join(model_dir, 'day_1.csv')):
                os.remove(path=os.path.join(model_dir, 'day_1.csv'))    
            # For day 2 to day 29, shift all of them by 1 to the left on number line, i.e day - 1
    ...
    def prepare_forecast_input(self, merged_prices_df: pd.DataFrame, model_id: int):
        ...
        forecast_input = forecast_input[-(self.INPUT_SIZE):, :]
    ...
    '''
    def fill_pertubed_dateset(self):
    ...
    def generate_forecasts_and_persist(self, model_id: int, merged_prices_df: pd.DataFrame, day: int, 
        ...
        if retrain_day == True:
            ...
            logging.debug(msg=f''' [TRAINING DAY]: For Model : [{model_id}], Day : {day + 1} 
            ...
            tcn_model.save(os.path.join(self.saved_model_dir, f'model_{model_id}'), include_optimizer=False) 
        ...
        logging.info(f'''Started forecast for model = {model_id} and day = {day + 1}''')
    ...
    def merge_forecasts_of_models(self): 
        ...
        for model_id in range(0, self.PERTUBED_MODEL_COUNT):
            # Day 0 remain unchanged, for all day >= 1 and <= 30 
            # Store all the 30 dataframes corresponds to the forecast of that particular day, use of this is later.
            ...
            ''' For each pertubed model we have 30 dataframes '''
    ...
    def forecast_and_recommend(self) -> None:
        ...
        training_end_date = datetime.strptime(self.training_start_date, self.DATE_FORMAT) + timedelta(days=delta.days - 30)
        model_retrain_date = training_end_date
        day: int = 0
        logging.debug(msg=f'''Day = {day}, Training data till : {training_end_date}, Model retrain date: {model_retrain_date}''') 
        ...
        if (os.path.exists(path=last_run_info_file)):
            ...
            with open(file=last_run_info_file, mode='rb') as f:
                training_end_date, model_retrain_date, day = pickle.load(file=f)    
            f.close() 
        ...
        for model_id in range(0, self.PERTUBED_MODEL_COUNT):
            ...
            merged_prices_df: pd.DataFrame = merged_prices_df[merged_prices_df['DATE'] >= self.training_start_date]    
            ''' Generate forecast through each model single original model + four pertubed models'''
            merged_prices_df = merged_prices_df[merged_prices_df['DATE'] < training_end_date] #TODO: check if this should be < or <= 
            merged_prices_df = merged_prices_df.reset_index(drop = True)
            ''' Generate and persist the forecast '''
            
            self.generate_forecasts_and_persist(model_id=model_id, merged_prices_df=merged_prices_df, 
                                                day=day, retrain_day=retrain_day,
                                                learning_rate=learning_rate, batch_size=batch_size)
        ...
        ''' If there exists enough file, atleast for last 30 days, then start with the recommendations '''
        ...
        with open(os.path.join(self.persist_data_dir, self.MODEL_FILE_INFO), mode='wb') as f:
            pickle.dump([training_end_date, model_retrain_date, day], file=f)
        f.close()
    ...
    def run_experiment(self, start_date, end_date) -> None:    
        ...
        total_days_to_run = (datetime.strptime(end_date, self.DATE_FORMAT) \
                                - datetime.strptime(start_date, self.DATE_FORMAT)).days + 30
        for day in range(0, total_days_to_run):
            logging.debug(msg=f'Forecast and Recommendation called for day = {day}\n\n')
    ...
    def forecast(self, till_date):
        ...
        already_trained_till = datetime.strptime(self.training_start_date, self.DATE_FORMAT) \
                                        + timedelta(days=delta.days - 30)
        ...
        if os.path.exists(previous_run_info_file):
            with open(file=previous_run_info_file, mode='rb') as f:
                already_trained_till, _, _ = pickle.load(file=f)   
            f.close()
        
        ''' Note that already_trained_till is updated and thus if till_date is same as already_trained_till, thus we have +1'''
        count_iterations = 1 if till_date == already_trained_till else (till_date - already_trained_till).days
        logging.info(f'Model will start forecasting from {already_trained_till} and will goes up till {till_date}')
        for day in range(0, count_iterations):
            forecast_date: str = datetime.strftime(already_trained_till + timedelta(days=day), self.DATE_FORMAT)
...
if __name__ == '__main__':
    ...
    tcn_model = TCNModel(target_state='rajasthan', target_mandi = 'kota',
                         surrogate_state='madhya pradesh', surrogate_mandi = 'mahidpur', 
                         lag_days=3, training_start_date='2006-01-01', recommendation_start_date='2006-07-01',
                         commodity='soyabean')
    
    # tcn_model.run_experiment(start_date = '2006-06-01', end_date = '2007-03-01')
    tcn_model.forecast(till_date='2007-01-01')
